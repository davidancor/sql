...
SELECT
  product_name,
  /* Extract description after hyphen or return NULL */
  NULLIF(
    TRIM(
      SUBSTR(
        product_name,
...
/* Revenue per vendor & product if every customer buys 5 units */

SELECT 
    v.vendor_name,
    p.product_name,
    5 * vi.original_price * COUNT(c.customer_id) AS total_revenue
FROM (
...
SET current_quantity = COALESCE(
    (
        ...
        ORDER BY vi.market_date DESC   -- use the column that defines "last"
        LIMIT 1
    ),
    0
);
